...
TIMEOUT = 20
MIN_DOMAIN = 1
MAX_DOMAIN = 3  # ⚠️ Paling aman, gak gampang dicancel
JEDA_ANTAR = [1.8, 2.2, 2.7]  # Jeda lebih alami
...
def proses_semua(daftar_domain, email, password):
    ...
    # Cek batas maksimal
    if len(daftar_domain) > MAX_DOMAIN:
        hasil["status"] = f"❌ Maksimal {MAX_DOMAIN} domain saja per proses"
        hasil["keterangan"] = "Kalau mau tambah lagi, ulangi proses dengan EMAIL BARU"
        return hasil

    if len(daftar_domain) < MIN_DOMAIN:
        hasil["status"] = f"❌ Minimal {MIN_DOMAIN} domain"
        return hasil
    ...
    for domain in daftar_domain:
        try:
            ...
            hasil["keterangan"] += f"⚠️ Gagal {domain}: {str(e)[:25]} | "
            continue

    if not domain_berhasil:
        hasil["status"] = "❌ Tidak ada domain yang berhasil dimasukkan"
        return hasil

    hasil["total_harga"] = f"Rp {total:,}"

    # Cek keranjang terisi
    if not cek_keranjang(sesi):
        hasil["status"] = "❌ Keranjang kosong, proses dibatalkan"
        return hasil
    ...
    try:
        ...
        if "viewinvoice.php" in res.text or "viewinvoice.php" in res.url:
            ...
            if inv:
                ...
                hasil["status"] = f"✅ BERHASIL | {len(domain_berhasil)} domain | 1 Invoice"
                hasil["keterangan"] += "\n💡 Bayar manual. Mau tambah lagi? Pakai EMAIL BARU ya!"
            ...
            pesan = (err.group(1) or err.group(2) or "Dibatalkan sistem")[:180]
            hasil["status"] = "❌ Gagal / Dibatalkan"
            hasil["keterangan"] += f"\n⚠️ Alasan: {pesan}"
...
<body class="bg-gray-100 min-h-screen p-4 md:p-8">
    <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-6">
        <h1 class="text-2xl font-bold text-center text-indigo-700 mb-2">📝 Aturan: Maks 3 Domain / Email</h1>
        <p class="text-center text-gray-600 mb-6">✅ Kalau mau tambah lagi, ulangi pakai email baru</p>

        <form id="formInput" class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah Domain (1 - 3 saja)</label>
                <input type="number" id="jumlah" min="1" max="3" value="2" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Daftar Domain</label>
                <textarea id="daftar_domain" rows="3" placeholder="Contoh:&#10;domain1.biz.id&#10;domain2.biz.id" class="w-full px-3 py-2 border rounded-lg" required></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Email Akun</label>
                <input type="email" id="email" placeholder="Gunakan email baru kalau sudah pernah" class="w-full px-3 py-2 border rounded-lg" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Password Akun</label>
                <input type="text" id="password" placeholder="Minimal 6 karakter" class="w-full px-3 py-2 border rounded-lg" required>
            </div>
            <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg font-medium">🚀 Proses & Buat Invoice</button>
        </form>
    ...
    <script>
        ...
        form.addEventListener('submit', async (e) => {
            e.preventDefault();
            hasilDiv.innerHTML = '<div class="text-center py-6 text-gray-600">⏳ Memproses... Tunggu sebentar ya!</div>';
            hasilDiv.classList.remove('hidden');

            const domains = document.getElementById('daftar_domain').value.split('\\n').map(d => d.trim()).filter(d => d);
            const jumlah = parseInt(document.getElementById('jumlah').value);

            if (domains.length !== jumlah || jumlah < 1 || jumlah > 3) {
                hasilDiv.innerHTML = `<div class="text-red-600 p-4 rounded-lg border">❌ Hanya boleh 1-3 domain per proses!</div>`;
                return;
            }
            ...
            try {
                const res = await fetch('/proses', {
                    ...
                    body: JSON.stringify({domains, email: document.getElementById('email').value, password: document.getElementById('password').value})
                });
                const h = await res.json();

                let html = `<h2 class="text-xl font-semibold mb-4">📋 Hasil Proses</h2>`;
                html += `<div class="p-4 rounded-lg border ${h.status.includes('✅') ? 'bg-green-50' : 'bg-red-50'}">`;
                html += `<p><strong>Status:</strong> ${h.status}</p>`;
                html += `<p><strong>Total Bayar:</strong> ${h.total_harga}</p>`;
                if (h.invoice_id !== '-') {
                    html += `<p><strong>Nomor Invoice:</strong> ${h.invoice_id}</p>`;
                    html += `<p><strong>Link Tagihan:</strong> <a href="${h.link_invoice}" target="_blank" class="text-blue-600 underline">${h.link_invoice}</a></p>`;
                    if (h.qris) html += `<p class="mt-2"><strong>QRIS Bayar:</strong><br><img src="data:image/png;base64,${h.qris}" class="w-36 h-36 mt-1 border p-1 rounded"></p>`;
                }
                html += `<p class="mt-3 text-sm text-gray-700"><strong>Catatan:</strong><br>${h.keterangan.replaceAll('|', '<br>')}</p>`;
                html += `</div>`;

                hasilDiv.innerHTML = html;

            } catch (err) {
                hasilDiv.innerHTML = `<div class="text-red-600 p-4 rounded-lg border">❌ Terjadi kesalahan: ${err}</div>`;
            }
        });
    </script>
</body>
...
def jalankan_proses():
    ...
    return jsonify(proses_semua(data.get('domains', []), data.get('email', ''), data.get('password', '')))
