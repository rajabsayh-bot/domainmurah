...
# ==============================================
# 📧 BUAT MAIL.TM FIX - PASTI DAFTAR
# ==============================================
def buat_mail_tm():
    max_coba = 8  # Coba sampai 8x kalau gagal
    for _ in range(max_coba):
        try:
            ...
            nama = f"tm_{''.join(random.choices(string.ascii_lowercase + string.digits, k=10))}"
            email = f"{nama}@{domain_tm}"

            # Kirim pendaftaran
            r = requests.post(
                f"{MAILTM_API}/accounts",
                json={"address": email, "password": PASS_MAILTM},
            ...
            if r.status_code in (200, 201):
                return email, PASS_MAILTM
            # Kalau sudah dipakai, coba nama lain
            elif "already used" in r.text.lower():
                time.sleep(0.3)
                continue
            # Kalau error lain, tunggu sebentar
            else:
                time.sleep(0.5)
                continue
...
def proses_domain(domain, sandi_akun):
    ...
    if not email_tm:
        hasil["status"] = "❌ Gagal daftar Mail.TM"
        return hasil
